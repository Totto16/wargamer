...
describe('Tankopedia', function () {
  ...
  describe('#findVehicleProfile()', function () {
    it('finds stock vehicle profiles', function () {
      return expect(
        client.tankopedia.findVehicleProfile(6673),
      ).to.eventually.have.deep.property('engine.name', 'Maybach HL 57 TR');
    });

    it('finds top vehicle profiles', function () {
      return expect(
        client.tankopedia.findVehicleProfile(1, 'top'),
      ).to.eventually.have.deep.property('gun.name', '76 mm S-54');
    });

    it('finds vehicle profiles by ID', function () {
      return expect(
        client.tankopedia.findVehicleProfile(1041, '21-1047-2066-4372'),
      ).to.eventually.have.deep.property('modules.radio_id', 1047);
    });

    it('returns null when no matches are found - stock profile', function () {
      return expect(client.tankopedia.findVehicleProfile(-1)).to.eventually.be
        .null;
    });

    it('returns null when no matches are found - top profile', function () {
      return expect(client.tankopedia.findVehicleProfile(-1, 'top')).to
        .eventually.be.null;
    });
    ...
    it('rejects when the profile ID is invalid', function () {
      return expect(
        client.tankopedia
          .findVehicleProfile(-1, '66-7777-8888-9999')
          .catch((error) => error),
      ).to.eventually.be.instanceof(Error);
    });
  });
  ...
  describe('#localizeCrewRole()', function () {
    it('localizes crew roles', function () {
      ...
      ).to.eventually.equal('Radio Operator');
    });
  });
  ...
  describe('#localizeVehicleNation()', function () {
    it('localizes vehicle types', function () {
      ...
      ).to.eventually.equal('Czechoslovakia');
    });
  });
});
