  ...
  findVehicleProfile(
    ...
    if (profile === 'stock') {
      return this.client
        ...
        .then(
          (response) => (response.data && response.data[vehicleId]) ?? null,
        );
    }
    if (profile === 'top') {
      return this.findVehicle(vehicleId)
        .then(
          ...
          ): Promise<APIResponse<Record<string, VehicleData>>> | null => {
            if (!vehicleData) {
              return null;
            }
            ...
            const queryFields = Object.keys(topModules).reduce(
              (accumulated, next) => {
                ...
                if (!fieldName) {
                  ...
                  throw new Error(`Field name '${next}' not found`);
                }
        ...
        .then((result): Record<string, unknown> | null => {
          ...
          return result.data[vehicleId] ?? null;
        });
    }
    ...
    return this.client
      ...
      .then((response) => (response.data && response.data[vehicleId]) ?? null);
  }
